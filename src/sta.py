...
from archipelago.app_graph_to_RTL import app_graph_to_RTL
...
def calc_sb_delay(graph, node, parent, comp, mem_column, sparse):
    # Need to associate each sb hop with these categories:
    # mem2pe_clk
    # pe2mem_clk
    # north_input_clk
    # south_input_clk
...
def main():
    ...
    placement = load_placement(placement_file) # SY: (Dumped from archipelago/pipeline.py)
    routing = load_routing_result(routing_file) # SY: (Dumped from archipelago/pipeline.py)
    ...
    # Call app_graph_to_RTL
    dirname = os.path.dirname(packed_file)
    app_graph_to_RTL(routing_result_graph, output_file=os.path.join(dirname, "application_graph.v"), data_width=16)
